 ...
 *
 * GitHub is configured to redirect here (FRONTEND_URL/auth/callback/github).
 * We receive the raw OAuth code + state nonce from GitHub, then forward them
 * to the backend's /auth/callback/github endpoint which:
...
export default function GitHubCallbackPage() {
  ...
  useEffect(() => {
    ...
    if (errorParam) {
      setError(`GitHub sign-in failed: ${errorParam}`);
    ...
    window.location.href = `${API_BASE_URL}/auth/callback/github?${params.toString()}`;
  }, [searchParams, router]);
